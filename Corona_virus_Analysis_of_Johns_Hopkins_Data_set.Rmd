...
The title of each table displays the number of columns and rows in the dataset. The U.S. data sets contain case and death counts, with the us_deaths dataset also including the population of each county in the U.S. The dataset contains and extensive list of reporting dates, which contain the daily case counts, deaths for the U.S. dataset and  cases, deaths, and recoveries for the global dataset. 
...
# Display the column names and the first few rows of the datasets with row and column counts
kable(global_confirmed_head, caption = paste("Global Confirmed Cases (Rows:", nrow(global_confirmed), ", Columns:", ncol(global_confirmed), ")"))
kable(global_deaths_head, caption = paste("Global Deaths Data (Rows:", nrow(global_deaths), ", Columns:", ncol(global_deaths), ")"))
kable(global_recovered_head, caption = paste("Global Recovered Data (Rows:", nrow(global_recovered), ", Columns:", ncol(global_recovered), ")"))
kable(us_confirmed_head, caption = paste("U.S. Confirmed Cases Data (Rows:", nrow(us_confirmed), ", Columns:", ncol(us_confirmed), ")"))
kable(us_deaths_head, caption = paste("U.S. Deaths Data (Rows:", nrow(us_deaths), ", Columns:", ncol(us_deaths), ")"))
...
For the global dataset, the total number of recovered cases is counted up to December 13, 2020. Due to this lack of information we calculated the recovered cases by subtracting the total deaths from the total cases. The calculations for total cases, deaths, and recoveries are presented in the table below.
...
country_comparison <- global_confirmed %>%
  ...
  theme(legend.position = "right")+
  geom_text(
    data = . %>% group_by(Country_Region) %>% filter(date == max(date)),
    aes(label = Country_Region),
    hjust = 2.75, vjust = -0.25,
    show.legend = FALSE,
  ) +
...
## References
John Hopkins University Center for Systems Science and Engineering (JHU CSSE) COVID-19 Data Repository. (2020). COVID-19 Data Repository by the Center for Systems Science and Engineering (CSSE) at Johns Hopkins University. <https://coronavirus.jhu.edu/>

GitHub. (2020). CSSEGISandData/COVID-19. <https://github.com/CSSEGISandData/COVID-19/tree/master/csse_covid_19_data/csse_covid_19_time_series>